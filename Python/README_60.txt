...
1) Running the script without any arguments (./targetscan_60.py) will print out a help screen.
1) Running the script without the '-h' flag (./targetscan_60.py -h) will print out a formats of input files.
2) Running the script with input filenames and output file will perform the analysis. Ex:
	./targetscan_60.py miR_Family_info_sample.txt UTR_Sequences_sample.txt targetscan_60_output.txt
...
This script was designed on a Linux platform. While running this script on Windows or Mac platforms, make sure to call the native python binary. 
This can be done explicitly by executing it as 'python targetscan_60.py' or changing the first line of the script to point to the native binary. 
